...
public class Frog extends Enemy {
    ...
    public void onDeath(Entity below) {
        ...
        if (checker(positionX, positionY)) {
	...
	/**
     * used by the method called upon an enemy.
     * sees if the tiles selected result in an outcome differing from the default
     * @param x int x position
     * @param y int y position
	 * @return boolean true if a space is valid false if a space is invalid
     */
    ...
    /**
     * Get the neighbours of a specific node which are not null designed to be used in move.
    ...
    public void move() {
        ...
        GraphNode[][] graphedLevelState =
                new GraphNode[game.MAX_HEIGHT_INDEX + 2][game.MAX_WIDTH_INDEX + 1];
        ...
        while (queue.size() > 0) {
            ...
            for (int i = 0; i < Neighbours.size(); i++) {
                if (Neighbours.get(i).getDistance() == -1) {
                    ...
                    graphedLevelState[Neighbours.get(i).getY()][Neighbours.get(i).getX()] =
                            Neighbours.get(i);
                }
            }
        }
        ...
        if (routeToPlayer) {
            ...
            game.updateLevel(currentNode.getX(), currentNode.getY(),
                    currentLevelState[getY()][getX()]);

            if (currentNode.getY() == playerCoordinates[0] &&
                    currentNode.getX() == playerCoordinates[1]) {
                Player.getPlayer().playerDeath();
            }

        } else if (!routeToPlayer) {
            //Moves in random direction when player is not detected
            //randomises by using shuffling arraylist and then getting
